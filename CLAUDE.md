...
> `make_choropleth_dashboard.py` and `how-to-do-custom-modifications.md` are referenced in older docs but **are not present in this repo**. Do not create them — `make_choropleth_chart.py` is the canonical implementation.
...
**Wide-format CSV** (input to Generate Dashboard): column 1 = region names, columns 2–5 = up to 4 numeric indicators (headers become KGI labels). An optional special row with the first column value `"National KPI"` (case-insensitive) is extracted before rendering and displayed as a reference benchmark in each panel — it is not plotted on the map.

**Raw KPI-entry CSV** (input to Shape Raw Data tab): must have columns `Cadre`, `Ministry/Department`, `KGI`, `Estimated figure`. Optional `Sector` column enables sector-level filtering (used for filtering only — not included in the pivoted output). Optional `National KPI` column, if present, is extracted per KGI and appended as the special marker row in the wide-format CSV download. Deduplication is on (`Cadre`, `KGI`), keeping first occurrence.
